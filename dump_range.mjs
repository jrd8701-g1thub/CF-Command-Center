...
async function dump() {
    ...
    for (let r of rows) {
        ...
        if (rn >= 165 && rn <= 210) {
            console.log(`${rn}: [${r.get('Transaction_ID')}] | [${r.get('Customer_Name')}] | [${r.get('Item_Name')}] | [${r.get('Order_Type')}] | [${r.get('Commission_Earned')}] | [${r.get('Delivery Status')}] | [${r.get('Driver_Name')}]`);
        }
    }
}
